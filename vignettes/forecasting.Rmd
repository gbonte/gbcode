...
for (f in 1:(NCOL(NN5)-1)){
  ...
  for (H in c(20,40,60,80,100)){
    ...
    for (i in I.ts){  ## looping over different test sets
      ...
      ## NAIVE
      p.naive<-multiplestepAhead(TimeSer,n,H,D,"stat_naive")
      M.naive<-c(M.naive,MASE(fut,p.arima))
      ...
      cat("\n  f= ", f, " i=", i, "H=", H, " M.arima=",mean(M.naive), " M.direct=",mean(M.direct), " M.mimo=",mean(M.mimo),
