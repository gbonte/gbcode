...
FF<-0  ## forgetting factor
n<-10  ##  embedding order
C<-5
D<-0
Kmin<-5  ## minimum number of neighbors
S<-1 ## initial position of the training set
...
for (f in 1:(NCOL(NN5)-1)){
  ...
  for (H in c(20,40,60,80,100)){
    ...
    for (i in I.ts){  ## looping over different test sets
      ...
      S.mimo.acf.lin<-c(S.mimo.acf.lin,SMAPE(fut,p.mimo.acf.lin,Cent,Sc))
