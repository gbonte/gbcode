...
rawTS=diff(rawTS) ## differentiation
assess=TRUE
visualize=TRUE
...
if (assess)
  for (H in c(20))
    for (i in seq(round(2*N/3),N-H,by=1)){
      ...
      Y.cont2=multiplestepAhead(TStr,n=n, H=H,method=method2,nunits=50,epochs=10,detrend=detrend)
      NMSE2=mean(TSts-Y.cont2)^2
      Y.cont3=multiplestepAhead(TStr,n=n, H=H,method=method3,nunits=50,epochs=10,detrend=detrend)
