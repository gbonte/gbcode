...
Fhat<-NULL
savefile<-"assessWEA.Rdata"


load("./data/WEAdata.Rdata")
Fhat<-NULL


for ( f in 1:length(WEAdata)){

    D=WEAdata[[f]]
    for (j in 1:NCOL(D))
        D[,j]=remNA(D[,j])
  wc=which(apply(D,2,sd)>0.01)
  D=D[,wc]
  ...
  n=4
  Nmax<-2000 ## max number of series samples
  mmax<-100  ## max number of series
  ...
  if (NROW(D)>Nmax){
      start=sample(1:(NROW(D)-Nmax),1)
    D=D[start:(start+Nmax-1),]
}  
  N=NROW(D)
  H=max(2,min(12,round(N/30)))
  ...
  E.hat11=NULL
  E.hat12=NULL
  methods=c("uni","VARs","dfm","dfm","dfml","dfml",
            "multifs2","multirr","multicca","lstm","multipls","uni")  
...
##    FF<-foreach( Ntr=NtrCV )%dopar%{
      ...
      if (season){
        ...
        bestS<-NULL
        for (i in 1:m){
          S=detectSeason(D2[1:Ntr,i],Ls=N,pmin=0.01,forced=TRUE, maxs=370)
      ...
      Xhat5=MmultiplestepAhead(Xtr,n,H,multi=methods[5],cdfml=3,
                               dfmlmodels=c("mimorr","stat_comb"))
      cat(".")
      Xhat6=MmultiplestepAhead(Xtr,n,H,multi=methods[6],cdfml=3,
                               dfmlmodels=c("multifs","stat_comb"))
      cat(".")
      ...
      cat(".")
      Xhat11=MmultiplestepAhead(Xtr,n,H,multi=methods[11],uni="lazydirect")
      cat(".")
      Xhat12=MmultiplestepAhead(Xtr,n,H,multi=methods[12],uni="mimocca")
      cat(".")
    ...
    cat( "ST",f,":: r=",r, "::", methods[1],":",mean(E.hat1)," | ")
  ...
  Fhat<-rbind(Fhat,cbind(numeric(length(E.hat1))+f,E.hat1,E.hat2,E.hat3,
                         E.hat4,E.hat5,
                         E.hat6,E.hat7,E.hat8,E.hat9,
                         E.hat10,E.hat11,E.hat12))
