...
for ( f in 1:length(STdata)){
  ...
  season=TRUE
  execute=TRUE
  methods=c("kfm","VARs","dfm","dfm","multifs2","multicca","uni","uni","multirr","uni")
  #methods=c("uni","VAR","VARs","uni","uni","uni","uni","multifs","uni","multifs2")
  
  
  
  if (NROW(D)>Nmax)
    D=D[(NROW(D)-Nmax):NROW(D),]
  ...
  for (r in 1:Nsets){
    ...
    for ( Ntr in round(seq(N-3*H,N-H,length.out=Lcv))){
      ...
      cat( f,":: r=",r, "::", methods[1],":",mean(E.hat1)," | ")
  ...
  save(file="assessST.Rdata",list=c("Fhat","Nmax","mmax","Lcv","Nsets"))
