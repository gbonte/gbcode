...
dirname="/home/gbonte/R/x86_64-pc-linux-gnu-library/3.6/gbcode/scripts/Forecasting/data"
DD=dir(dirname)
Fhat<-NULL
...
for ( f in 1:length(DD)){
  ...
  methods=c("uni","VARs","dfm","dfm","dfml","dfml",
                  "multifs2","multirr","multicca","lstm","uni","uni")  
  for (r in 1:Nsets){
    set.seed(f+r)
    D2=D
    for (i in 1:NCOL(D2))
...
#                  "multifs2","multirr","multicca","lstm","uni","uni")
    ...
    cat( DD[f],":: r=",r, "::", methods[1],":",mean(E.hat1)," | ")
