      ...
      tabItem(tabName = "Nonlinear",
              fluidRow(box(width=3,collapsible = TRUE,
                           selectInput("ord", label = h3("Functions"), 
                                       choices = list( "Pol 3" = 3, "sin 2x" = -1, "cos(3x)"=-2,"3/(1+x^2)"=-3), 
                                       selected = -2),
                           sliderInput("h","# RBF:", min = 2,max = 10, 
                       ...
                       box(width=7,collapsible = TRUE,title = "Sampling distribution",plotOutput("nlinearBV", height = 300))
              ),## fluidRow
              fluidRow(   
...
server<-function(input, output,session) {
  ...
  hyp<-function(X,Y, Xts, h){
    ...
    if (h==0){
      ...
      return(numeric(Nts)+mean(Y))
    }
    
    
   
    
    p<-rbftrain(X,Y,Xts,h)
