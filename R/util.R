...
unscalin<- function(scale.data,mean.data,std.data){
 ...
 if (no.var>1){
  ...
  data
                                        ...
                                        #(c) Gianluca Bontempi, January 1997
#
#	[INPUT,OUTPUT]=MakeInput(TS, N, DELAY) 
# 	version for multivariable time series 
# 	TS ... time series [no_data, no_var] 
# 	N  ... model order  [no_var,1] = vector of order for each variable 
# 	DELAY... [no_var,1] vector of delay for each variable 
# 
# 	INPUT(t,...)=ts(t-1:n(j)-delay(j),j) 
# 	OUTPUT(t,j)=ts(t,j) 
# Example 
#	data = 
# 
...
## n: number of unmarked elements
## k: selection size
## x: number of marked elements in our selection
hyperg<-function(m,n,k,x){
  p<-0
  for (i in x:min(k,m))
    #p<-p+choose(m,i)*choose(n,k-i)/choose(m+n,k)
    p<-p+dhyper(i,m,n,k)
  p
}
# p: pvalue of selecting >= x marked elements
...
var2H<-function(v){
  ## var2H: variance to entropy inthe Gaussian case
  ## 14/11/2011
  1/2*(1+log(2*pi*v))
}
